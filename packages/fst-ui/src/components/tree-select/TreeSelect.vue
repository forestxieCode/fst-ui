<template>
  <div class="fst-tree-select" :class="selectClass">
    <!-- 下拉文本 -->
    <el-select
...
<script>
import { sOn, sOff } from '../../utils/dom'
import { each, guid } from '../../utils/util'
export default {
  name: 'fst-tree-select',
  props: {
  ...
  mounted() {
    this._updateH()
    this.$nextTick(() => {
      sOn(document, 'mouseup', this._popoverHideFun)
    })
  },
  methods: {
    // 根据类型判断单选，多选
  ...
  components: {},
  beforeDestroy() {
    sOff(document, 'mouseup', this._popoverHideFun)
  }
}
</script>
